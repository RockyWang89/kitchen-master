...
function FoodItem({day, mealtime, id, name}) {
    ...
    return <div role="button" onClick={goToDetail} className="relative w-full food-item">
        <p className="bg-yellow-100 rounded p-[6px] text-left">{name}</p>
        <button onClick={removeFromCalendar} className="absolute top-[-8px] right-[-8px] bg-white rounded-[50%]">
